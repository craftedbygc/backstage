 ...
 * the core bundle (everything exported from `@unseenco/theatre-core`) to window.__TheatreJS_CoreBundle.
 */
function registerCoreBundle() {
  ...
  if (typeof existingBundle !== 'undefined') {
    if (
      ...
      is bundling `@unseenco/theatre-core` multiple times, but, there are legitimate scenarios where a user may want
      to include multiple instances of `@unseenco/theatre-core` on the same page.
