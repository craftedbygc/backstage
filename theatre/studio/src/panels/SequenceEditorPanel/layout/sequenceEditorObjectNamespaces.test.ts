...
describe('sequenceEditorObjectNamespaces', () => {
  ...
  test('buildSequenceEditorNamespaceMap nests GSAP proxies by object key', () => {
    const map = buildSequenceEditorNamespaceMap([
      ...
      mockSheetObject('Regular Theatre Object'),
    ])

    expect(map.has('Regular Theatre Object')).toBe(true)
    expect(map.get('Regular Theatre Object')?.object?.address.objectKey).toBe(
      'Regular Theatre Object',
    )
