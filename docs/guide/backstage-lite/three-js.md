...
[`@unseenco/backstage/threejs`](/guide/extensions/threejs.md) supports both full Backstage and lite: `autoAddObject`, `autoAddMaterial`, and `autoAddCamera` only need `ISheet` and `onValuesChange`, so they work with `@unseenco/backstage/core-lite`.
...
See the [Three.js extension guide](/guide/extensions/threejs.md) for `configureBackstageThreejs`, shared materials, and orbit mode—the APIs are the same; only package names change.
