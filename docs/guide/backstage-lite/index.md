...
**Backstage Lite** is a smaller Backstage.js stack for apps that ship **static prop values** and **sheet-level variants**, without timelines, keyframes, or sequence playback in production.
...
Quick API notes live in the package READMEs under `backstage/core-lite/` and `backstage/studio-lite/` in the monorepo. This guide is the narrative walkthrough.

## Bundle size

Sizes are measured with esbuild (dataverse external). Reproduce from the monorepo after `yarn workspace backstage build:js`:
...
- Same project id, object keys, prop schemas, and **`OnDiskState`** JSON shape as full Backstage (lite state is a valid subset).
