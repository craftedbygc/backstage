...
The API names match full Backstage for upgrade compatibility: `declareSequenceVariants`, `setActiveSequenceVariant`, `getActiveSequenceVariant`.
...
Full Backstage also stores **keyframes per variant** in `sequencesById`. Lite does not author or play those tracks; see [Sheet sequence variants](../manual/sheet-variants.md) for the full feature set.
...
[`sheet.props()`](../manual/sheets.md#sheet-level-props) are shared across variants—they are not duplicated per variant folder (same as full Backstage).
...
[`declareSequenceVariants`](/api/backstage-core), [`setActiveSequenceVariant`](/api/backstage-core), [`getActiveSequenceVariant`](/api/backstage-core) — signatures match full core; use the lite packages in lite apps.
