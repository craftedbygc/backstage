# Upgrading lite to full Backstage

Backstage Lite is designed so you can ship static JSON first and adopt timelines later without rewriting object addresses or prop schemas.
...
Full Backstage can attach a **separate timeline per variant** (`sequencesById[variantId]`). Lite-only projects have empty or stripped sequence data; you can build per-variant timelines after upgrade without migrating static layers.
