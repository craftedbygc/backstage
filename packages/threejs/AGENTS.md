...
| `src/autoAddObject.ts` | Register Three.js objects on Backstage sheets |
| `src/autoAddMaterial.ts` | Register Three.js materials on Backstage sheets (material props only) |
...
| `src/config.ts` | Project-wide `configureBackstageThreejs()` defaults |
| `src/colorUtils.ts` | Linear ↔ sRGB color conversion for Backstage rgba props |
...
| `src/types.ts` | Local `BackstageExtension` / toolbar types (avoids pulling full Studio types at build time) |
...
Register a Three.js object on a Backstage sheet with auto-parsed transforms and material properties:
...
const sheetObject = autoAddObject(mesh, sheet, {
  ...
  exclude: {uniforms: ['uNoise']}, // merged with configureBackstageThreejs defaults
  include: [],             // whitelist material props/uniforms
  trackMaterial: true,     // default: true when mesh has material
})
```

`autoAddObject` calls `sheet.object()` without `reconfigure` — same `objectKey` returns the existing instance per Backstage's normal rules. It registers the Object3D in an internal registry used by `buildExtension` for bidirectional selection.
...
2. A dedicated Backstage object is created under `Shared Materials / <material.name>`
...
Register a Three.js material on a Backstage sheet with auto-parsed material properties only (no transforms, no selection registry):
...
const sheetObject = autoAddMaterial(material, sheet, {
  ...
  namespace: '',
  exclude: {uniforms: ['uTime']}, // merged with configureBackstageThreejs defaults
  include: [],
})
```
...
Material texture slots (`map`, `normalMap`, etc.) and shader uniform textures are registered as Backstage `types.image` props with `persist: false`, so assignments apply for the current session only and are cleared on refresh. They are also marked **transient** (excluded from exported project state JSON).

## Selection sync

When both `autoAddObject` and `buildExtension` are used:

- **Outline → viewport**: selecting a Backstage object shows a `BoxHelper` on the matching Three.js object
...
Devtools state is stored on a **Studio project sheet** (`studio.getStudioProject().sheet('Extension: backstage-threejs')`), one object per scene (`Devtools: <scene name>`):
...
`backstage/core/src/projects/initialiseProjectState.ts` uses `??=` when initialising ahistoric state so extension sheet data is not wiped on project attach.

## Remote editor behaviour

The package depends on Studio remote-editor APIs in `backstage/studio/src/remoteEditor.ts`:
...
Studio UI visibility: the main window calls `studio.ui.hide()` when opening remote; persisted visibility must not hide Studio in the remote window (see `backstage/studio/src/Studio.ts` and `UIRoot.tsx`).
...
- Publish both CJS (`dist/index.js`, `dist/extension.js`) and ESM (`dist/index.mjs`, `dist/extension.mjs`) via `exports` so Vite/Nuxt share the consumer's `three` peer instead of nesting a second copy during CJS prebundling. Keep `three` / Backstage peers external in esbuild; target `es2020`.
...
- `@unseenco/backstage/studio` must not import `@unseenco/backstage` value exports (lint rule) — remote-editor helpers live in `backstage/studio/src/remoteEditor.ts` with a local `isRemoteEditorWindow()` duplicate.
...
- `backstage/studio/src/remoteEditor.ts` — remote editor open/close tracking (extracted from `GlobalToolbar.tsx`)
- `backstage/core/src/coreExports.ts` — exports `isRemoteEditorWindow`
- `backstage/studio/src/index.ts` — exports `isRemoteEditorOpen`, `onRemoteEditorOpenChange`
...
2. Respect main vs remote editor isolation — remote is for editing Backstage objects; the main window keeps rendering the scene.
