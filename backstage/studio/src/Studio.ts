import Scrub from '@unseenco/backstage/studio/Scrub'
import type {StudioHistoricState} from '@unseenco/backstage/studio/store/types/historic'
import UI from '@unseenco/backstage/studio/UI/UI'
import type {Pointer, Ticker} from '@unseenco/backstage/dataverse'
import {Atom, PointerProxy, pointerToPrism} from '@unseenco/backstage/dataverse'
import type {
  CommitOrDiscard,
  ITransactionPrivateApi,
} from './StudioStore/StudioStore'
import StudioStore from './StudioStore/StudioStore'
import type {IExtension, IStudio} from './BackstageStudio'
import BackstageStudio from './BackstageStudio'
import {nanoid} from 'nanoid/non-secure'
import type Project from '@unseenco/backstage/projects/Project'
import type {CoreBits} from '@unseenco/backstage/CoreBundle'
...
import type * as _coreExports from '@unseenco/backstage/coreExports'
...
import {setStudioAccentHex} from '@unseenco/backstage/studio/uiComponents/studioTokens'
import {
  isBackstageLiteStudioLocked,
  setRuntimeStudioMode,
} from '@unseenco/backstage/studio/utils/backstageLiteMode'

const DEFAULT_PERSISTENCE_KEY = 'backstage-0.4'
...
Backstage.js projects remain in pending mode (won't play their sequences) until the studio is initialized, so you should place the \`studio.initialize()\` line right after the import line:
...
export class Studio {
  ...
  /**
   * An atom holding the exports of '\@unseenco/backstage'. Will be undefined if '\@unseenco/backstage' is never imported
   */
  ...
  /**
   * This will be set as soon as `@unseenco/backstage` registers itself on `@unseenco/backstage/studio`
   */
  ...
  constructor() {
    ...
    this.publicApi = new BackstageStudio(this)
  ...
  async initialize(opts?: Parameters<IStudio['initialize']>[0]) {
    if (!this._coreBits) {
      throw new Error(
        `You seem to have imported \`@unseenco/backstage/studio\` without importing \`@unseenco/backstage\`. Make sure to include an import of \`@unseenco/backstage\` before calling \`studio.initializer()\`.`,
      )
    }
    ...
    if (opts?.mode === 'full' && isBackstageLiteStudioLocked()) {
      console.warn(
    ...
    const studioMode =
      isBackstageLiteStudioLocked() || opts?.mode === 'lite' ? 'lite' : 'full'
    ...
    if (opts?.__experimental_rafDriver) {
      if (
        opts.__experimental_rafDriver.type !== 'Backstage_RafDriver_PublicAPI'
      ) {
        throw new Error(
      ...
      if (!rafDriverPrivateApi) {
        ...
        throw new Error(
          'parameter `rafDriver` in `studio.initialize({__experimental_rafDriver})` seems to come from a different version of `@unseenco/backstage` than the version that is attached to `@unseenco/backstage/studio`',
        )
      }
      ...
      extension.panes?.forEach((classDefinition) => {
        ...
        if (existing) {
          if (reconfigure && existing.extensionId === extension.id) {
            ...
            console.warn(
              `Pane class "${classDefinition.class}" already exists. This is a bug in Backstage.js. Please report it at https://github.com/craftedbygc/backstage/issues/new`,
            )
          } else {
            throw new Error(
