...
import type {PaneInstance} from './BackstageStudio'
...
export default class PaneManager {
  ...
  createPane<PaneClass extends string>(
    ...
    if (!core) {
      throw new Error(
        `Can't create a pane because @unseenco/backstage is not yet loaded`,
      )
    }
  ...
  destroyPane(instanceId: PaneInstanceId): void {
    ...
    if (!core) {
      throw new Error(
        `Can't do this yet because @unseenco/backstage is not yet loaded`,
      )
    }
