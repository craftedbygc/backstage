import UIRoot from '@unseenco/backstage/studio/UIRoot/UIRoot'
import type {$IntentionalAny} from '@unseenco/backstage-shared/utils/types'
import React from 'react'
import ReactDOM from 'react-dom/client'
import {getMounter} from '@unseenco/backstage/studio/utils/renderInPortalInContext'
import {withStyledShadow} from '@unseenco/backstage/studio/css'
import ExtensionToolbar from '@unseenco/backstage/studio/toolbars/ExtensionToolbar/ExtensionToolbar'
...
export default class UINonSSRBits {
  ...
  constructor() {
    // @todo we can't bootstrap Backstage.js (as in, to design Backstage.js using backstage), if we rely on IDed elements
    this.containerEl.id = 'backstagejs-studio-root'
    ...
    const createShadowRoot = (): ShadowRoot & HTMLElement => {
      if (window.__IS_VISUAL_REGRESSION_TESTING === true) {
        ...
        fauxRoot.id = 'backstagejs-faux-shadow-root'
