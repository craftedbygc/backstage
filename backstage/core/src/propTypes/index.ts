...
/**
 * Prop type factory functions (exported from `@unseenco/backstage` as the `types` namespace).
 *
...
>(
  ...
  const config: PropTypeConfig_Compound<Props> = {
    ...
    [propTypeSymbol]: 'BackstagePropType',
    label: opts.label,
...
/** Base shape shared by all Backstage prop type configurations. */
...
> {
   ...
   */
  valueType: ValueType
  /** Internal marker distinguishing Backstage prop configs from plain values. */
  [propTypeSymbol]: 'BackstagePropType'
  /**
...
/** Any Backstage prop type configuration (simple, compound, or enum). */
