...
import type BackstageProject from '@unseenco/backstage/projects/BackstageProject'
import type {ISheetOptions} from '@unseenco/backstage/projects/BackstageProject'
import {applySheetRuntimeOptions} from '@unseenco/backstage/sheets/applySheetRuntimeOptions'
import type Sheet from '@unseenco/backstage/sheets/Sheet'
import SheetTemplate from '@unseenco/backstage/sheets/SheetTemplate'
...
import {PointerProxy} from '@unseenco/backstage/dataverse'
...
import RemoteSync from '@unseenco/backstage/internal/RemoteSync'
...
import {_coreLogger} from '@unseenco/backstage/_coreLogger'
...
export default class Project {
  ...
  type: 'Backstage_Project' = 'Backstage_Project'
  readonly _logger: ILogger

  constructor(
    id: ProjectId,
    readonly config: Conf = {},
    readonly publicApi: BackstageProject,
  ) {
    ...
    if (config.state) {
      setTimeout(() => {
        // The user has provided config.state but in case @unseenco/backstage/studio is loaded,
      ...
      if (typeof window === 'undefined') {
        if (process.env.NODE_ENV === 'production') {
          console.error(
            `Argument config.state in Backstage.getProject("${id}", config) is empty. ` +
              ...
              `otherwise your project's state will be empty and nothing will animate. Learn more at https://backstage.unseen.co/docs/guide/manual/projects#state`,
          )
        }
      } else {
        setTimeout(() => {
          if (!this._studio) {
            throw new Error(
              `Argument config.state in Backstage.getProject("${id}", config) is empty. This is fine ` +
                `while you are using @unseenco/backstage along with @unseenco/backstage/studio. But since @unseenco/backstage/studio ` +
                `is not loaded, the state of project "${id}" will be empty.\n\n` +
                `To fix this, you need to add @unseenco/backstage/studio into the bundle and export ` +
                `the project's state. Learn how to do that at https://backstage.unseen.co/docs/guide/manual/projects#state\n`,
            )
          }
        }, 1000)
      }
