...
import type {IRafDriver} from '@unseenco/backstage/rafDrivers'
...
export interface ISequence {
  /** Discriminator for Backstage.js public sequence instances. */
  readonly type: 'Backstage_Sequence_PublicAPI'
  ...
  play(conf?: {
     ...
     * Learn how to use `rafDriver`s [from the docs](https://backstage.unseen.co/docs/guide/manual/advanced#rafdrivers).
     */
   ...
   * ```
   *
   * Note: It's better to provide the `audioContext` rather than allow Backstage.js to create it.
   * That's because some browsers [suspend the audioContext](https://developer.chrome.com/blog/autoplay/#webaudio)
   * unless it's initiated by a user gesture, like a click. If that happens, Backstage.js will
  ...
  attachAudio(args: IAttachAudioArgs): Promise<{
    ...
    /**
     * This is an intermediate GainNode that Backstage.js feeds its audio to. It is by default
...
export default class BackstageSequence implements ISequence {
  get type(): 'Backstage_Sequence_PublicAPI' {
    return 'Backstage_Sequence_PublicAPI'
  }
      ...
      if (process.env.NODE_ENV !== 'production') {
        notify.warning(
          ...
          'You seem to have called `sequence.play()` before the project has finished loading.\n\n' +
            'This would **not** a problem in production when using `@unseenco/backstage`, since Backstage.js loads instantly in core mode. ' +
            "However, when using `@unseenco/backstage/studio`, it takes a few milliseconds for it to load your project's state, " +
          ...
          [
            {
              url: 'https://backstage.unseen.co/docs/api/backstage-core#project.ready',
              title: 'Project.ready',
            },
          ],
        )
