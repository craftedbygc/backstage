...
import {Atom, pointer} from '@unseenco/backstage/dataverse'
import type {
  GsapClipTrack,
...
import type {ISequence} from './BackstageSequence'

const LITE_DEFAULT_LENGTH = 10

/**
 * Inert `ISequence` for `@unseenco/backstage/core-lite` (position 0, no playback).
 */
export default class BackstageSequenceLite implements ISequence {
  readonly type = 'Backstage_Sequence_PublicAPI' as const
  ...
  attachAudio(_args: {
    ...
    return Promise.reject(
      new Error(
        'sequence.attachAudio() is not available in @unseenco/backstage/core-lite',
      ),
    )
  }
}
