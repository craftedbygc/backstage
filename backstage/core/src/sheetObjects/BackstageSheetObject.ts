...
import {onChange} from '@unseenco/backstage/coreExports'
...
> {
  /**
   * All Objects will have `object.type === 'Backstage_SheetObject_PublicAPI'`
   */
  readonly type: 'Backstage_SheetObject_PublicAPI'
...
// Enabled for https://linear.app/backstage/issue/P-217/if-objvalue-is-read-make-sure-its-derivation-remains-hot-for-a-while
// Disable to test old behavior
const KEEP_HOT_FOR_MS: undefined | number = 5 * 1000

export default class BackstageSheetObject<
  Props extends UnknownShorthandCompoundProps = UnknownShorthandCompoundProps,
> implements ISheetObject<Props>
{
  get type(): 'Backstage_SheetObject_PublicAPI' {
    return 'Backstage_SheetObject_PublicAPI'
  }
  private readonly _cache = new SimpleCache()
  /** @internal See https://linear.app/backstage/issue/P-217/if-objvalue-is-read-make-sure-its-derivation-remains-hot-for-a-while */
  ...
  get value(): PropsValue<Props> {
    ...
    if (KEEP_HOT_FOR_MS != null) {
      ...
      if (this._keepHotUntapDebounce) {
        ...
        // See https://linear.app/backstage/issue/P-217/if-objvalue-is-read-make-sure-its-derivation-remains-hot-for-a-while
