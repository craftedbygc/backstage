...
import type {IProject, IProjectConfig} from './projects/BackstageProject'
import BackstageProject from './projects/BackstageProject'
...
import type {PointerType, Prism} from '@unseenco/backstage/dataverse'
import {isPointer} from '@unseenco/backstage/dataverse'
import {isPrism, pointerToPrism} from '@unseenco/backstage/dataverse'
import type {
  $IntentionalAny,
  VoidFn,
...
export function getProject(id: string, config: IProjectConfig = {}): IProject {
  ...
  if (existingProject) {
    if (process.env.NODE_ENV !== 'production') {
      if (!deepEqual(config, existingProject.config)) {
        throw new Error(
          `You seem to have called Backstage.getProject("${id}", config) twice, with different config objects. ` +
            `This is disallowed because changing the config of a project on the fly can lead to hard-to-debug issues.\n\n` +
            `You can fix this by either calling Backstage.getProject() once per projectId,` +
  ...
  if (process.env.NODE_ENV !== 'production') {
    validateName(id, 'projectName in Backstage.getProject(projectName)', true)
  ...
  return new BackstageProject(id, config)
}
...
const shallowValidateOnDiskState = (projectId: ProjectId, s: OnDiskState) => {
  if (
    ...
    throw new InvalidArgumentError(
      `Error validating conf.state in Backstage.getProject(${JSON.stringify(
        projectId,
      )}, conf). The state seems to be formatted in a way that is unreadable to Backstage.js. Read more at https://www.backstagejs.com/docs/latest/manual/projects#state`,
    )
  }
}
...
const validateProjectNumberPrecisionOrThrow = (
  ...
  if (
    ...
    throw new InvalidArgumentError(
      `Argument config.numberPrecision in Backstage.getProject(projectId, config) must be a non-negative integer. ${userReadableTypeOfValue(
        numberPrecision,
      )} given.`,
    )
  }
...
const validateProjectIdOrThrow = (value: string) => {
  if (typeof value !== 'string') {
    throw new InvalidArgumentError(
      `Argument 'projectId' in \`Backstage.getProject(projectId, ...)\` must be a string. Instead, it was ${userReadableTypeOfValue(
        value,
      )}.`,
    )
  }
  ...
  if (idTrimmed.length !== value.length) {
    throw new InvalidArgumentError(
      `Argument 'projectId' in \`Backstage.getProject("${value}", ...)\` should not have surrounding whitespace.`,
    )
  }

  if (idTrimmed.length < 3) {
    throw new InvalidArgumentError(
      `Argument 'projectId' in \`Backstage.getProject("${value}", ...)\` should be at least 3 characters long.`,
    )
  }
}
 ...
 * @param rafDriver - (optional) The `rafDriver` to use. Learn how to use `rafDriver`s [from the docs](https://www.backstagejs.com/docs/latest/manual/advanced#rafdrivers).
 * @returns An unsubscribe function
 *
 * @example
 * Usage:
 * ```ts
 * import {getProject, onChange} from '@unseenco/backstage'
 *
...
/**
 * @internal Used by extension packages (`@unseenco/backstage/gsap`, etc.). Not part of the
 * public API; exported only so published subpath `./privateAPIs` shares the main bundle singleton.
 */
export {privateAPI, setPrivateAPI, getBackstageCoreRafDriver} from './privateAPIs'
