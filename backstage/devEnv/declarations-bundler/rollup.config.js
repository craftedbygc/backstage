  ...
  return {
    ...
    external: (id) => {
      if (
        ...
        id.startsWith(
          `@unseenco/backstage-${which === 'studio' ? 'core' : 'studio'}`,
        )
      ) {
        return true
      }

      if (id.startsWith('@unseenco/backstage')) {
        return false
      }
      ...
      }),
      {
        ...
        generateBundle(_options, bundle) {
          ...
          if (!chunk || chunk.type !== 'chunk') {
            throw new Error(
              'Declarations bundler did not emit backstage/core/dist/index.d.ts',
            )
          }
