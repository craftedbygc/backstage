...
function writeCorePrivateAPIsShim(pathToPackage: string) {
  ...
  fs.writeFileSync(
    ...
    `export { privateAPI, setPrivateAPI, getBackstageCoreRafDriver, peekBackstageCoreRafDriver } from './index.mjs';\n`,
  )
  fs.writeFileSync(
    path.join(dist, 'privateAPIs.js'),
    `'use strict';\nconst index = require('./index.js');\nexports.privateAPI = index.privateAPI;\nexports.setPrivateAPI = index.setPrivateAPI;\nexports.getBackstageCoreRafDriver = index.getBackstageCoreRafDriver;\nexports.peekBackstageCoreRafDriver = index.peekBackstageCoreRafDriver;\n`,
  )
  fs.writeFileSync(
    path.join(dist, 'privateAPIs.d.ts'),
    `export { privateAPI, setPrivateAPI, getBackstageCoreRafDriver, peekBackstageCoreRafDriver } from './index';\n`,
  )
}
...
/**
 * Compare bundle sizes (run after `yarn workspace backstage build:js`).
 * Set `BACKSTAGE_LITE_LOG_BUNDLE_SIZES=1` to print sizes when building.
 *
 ...
 */
async function logBackstageLiteBundleSizesIfRequested() {
  if (process.env.BACKSTAGE_LITE_LOG_BUNDLE_SIZES !== '1') return
  ...
  for (const [pkg, base] of pairs) {
    ...
    for (const ext of ['js', 'mjs'] as const) {
      ...
      if (fs.existsSync(file)) {
        ...
        console.log(`[backstage-lite sizes] ${pkg}/${base}.${ext}: ${kb} KiB`)
      }
    }
  }
  ...
  console.log(
    '[backstage-lite sizes] --- unminified / minified (index.js, esbuild) ---',
  )

  for (const {label, entry, backstageLite, which} of metrics) {
    ...
    const sharedConfig: Parameters<typeof esbuild.build>[0] = {
      ...
      define: {
        ...definedGlobals,
        __BACKSTAGE_LITE__: backstageLite ? 'true' : 'false',
    ...
    if (which === 'core') {
      ...
      if (backstageLite) {
        ...
        sharedConfig.plugins = [
          {
            name: 'backstage-core-lite-stubs',
            setup(build) {
    ...
    console.log(
      `[backstage-lite sizes] ${label}: unminified ${formatKiB(
        unminBytes,
...
export async function createBundles(watch: boolean) {
  ...
  for (const target of targets) {
    ...
    const esbuildConfig: Parameters<typeof esbuild.context>[0] = {
      ...
      define: {
        ...definedGlobals,
        __BACKSTAGE_LITE__: target.backstageLite ? 'true' : 'false',
        __IS_VISUAL_REGRESSION_TESTING: 'false',
      },
      external: [
  ...
  if (!watch) {
    ...
    try {
      await logBackstageLiteBundleSizesIfRequested()
    } catch (err) {
      console.warn('[backstage-lite sizes] extended metrics failed:', err)
    }
  }
}
