import type {
  ...
  _LogFns,
  IBackstageInternalLoggerOptions,
  IUtilLogger,
  ILogger,
} from './logger'
import {createBackstageInternalLogger} from './logger'
...
function setupFn(options: IBackstageInternalLoggerOptions) {
  const con = spyConsole()
  const internal = createBackstageInternalLogger(con, options)
  function t(logger = internal.getLogger()) {
    return {
      ...
      expectIncluded(
        kind: keyof _LogFns,
        expectOutputted: keyof IBackstageConsoleLogger,
...
function setupUtilLogger(
  ...
  con: jest.Mocked<IBackstageConsoleLogger>,
) {
  return {
...
function spyConsole(): jest.Mocked<IBackstageConsoleLogger> {
  return {
    debug: jest.fn(),
    info: jest.fn(),
    error: jest.fn(),
