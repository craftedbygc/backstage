...
export interface IBackstageLogger {
  error(level: IBackstageLogMeta, message: string, args?: Loggable): void
  warn(level: IBackstageLogMeta, message: string, args?: Loggable): void
  debug(level: IBackstageLogMeta, message: string, args?: Loggable): void
  trace(level: IBackstageLogMeta, message: string, args?: Loggable): void
}

type IBackstageLogMeta = Readonly<{
  audience: 'public' | 'dev' | 'internal'
  category: 'general' | 'todo' | 'troubleshooting'
  level: BackstageLoggerLevel
}>

/** @public configuration type */
export interface IBackstageConsoleLogger {
  /** ERROR level logs */
...
export type IBackstageLoggerConfig =
  ...
  | {
      ...
      style?: boolean
      /** default {@link console} */
      console?: IBackstageConsoleLogger
    }
  | {
      type: 'named'
      named(names: string[]): IBackstageLogger
    }
  | {
      type: 'keyed'
      keyed(
...
export type IBackstageLogSource = {names: {name: string; key?: number | string}[]}

export type IBackstageLogIncludes = {
  /**
   * General information max level.
   * e.g. `Project imported might be corrupted`
   */
  min?: BackstageLoggerLevel
  /**
   * Include logs meant for developers using Backstage.js
  ...
  dev?: boolean
  /**
   * Include logs meant for internal development of Backstage.js
...
export type IBackstageLoggingConfig = IBackstageLogIncludes & {
  include?: (source: IBackstageLogSource) => IBackstageLogIncludes
...
/** @internal */
enum _Audience {
  /** Logs for developers of Backstage.js */
  INTERNAL = 1 << 3,
  /** Logs for developers using Backstage.js */
  DEV = 1 << 4,
  /** Logs for users of the app using Backstage.js */
  PUBLIC = 1 << 5,
}

export enum BackstageLoggerLevel {
  TRACE = 1 << 6,
  DEBUG = 1 << 7,
  WARN = 1 << 8,
  ERROR = 1 << 9,
...
/**
 * @internal Backstage.js internal "dev" levels are odd numbers
 *
 * You can check if a level is odd quickly by doing `level & 1 === 1`
 */
export enum _LoggerLevel {
  /** The highest logging level number. */
  ERROR_PUBLIC = BackstageLoggerLevel.ERROR |
    _Audience.PUBLIC |
    _Category.GENERAL,
  ERROR_DEV = BackstageLoggerLevel.ERROR | _Audience.DEV | _Category.GENERAL,
  /** @internal this was an unexpected event */
  _HMM = BackstageLoggerLevel.ERROR |
    _Audience.INTERNAL |
    _Category.TROUBLESHOOTING,
  _TODO = BackstageLoggerLevel.ERROR | _Audience.INTERNAL | _Category.TODO,
  _ERROR = BackstageLoggerLevel.ERROR | _Audience.INTERNAL | _Category.GENERAL,
  WARN_PUBLIC = BackstageLoggerLevel.WARN | _Audience.PUBLIC | _Category.GENERAL,
  WARN_DEV = BackstageLoggerLevel.WARN | _Audience.DEV | _Category.GENERAL,
  /** @internal surface this in this moment, but it probably shouldn't be left in the code after debugging. */
  _KAPOW = BackstageLoggerLevel.WARN |
    _Audience.INTERNAL |
    _Category.TROUBLESHOOTING,
  _WARN = BackstageLoggerLevel.WARN | _Audience.INTERNAL | _Category.GENERAL,
  DEBUG_DEV = BackstageLoggerLevel.DEBUG | _Audience.DEV | _Category.GENERAL,
  /** @internal debug logs for implementation details */
  _DEBUG = BackstageLoggerLevel.DEBUG | _Audience.INTERNAL | _Category.GENERAL,
  /** trace logs like when the project is saved */
  TRACE_DEV = BackstageLoggerLevel.TRACE | _Audience.DEV | _Category.GENERAL,
  /**
   * The lowest logging level number.
   * @internal trace logs for implementation details
   */
  _TRACE = BackstageLoggerLevel.TRACE | _Audience.INTERNAL | _Category.GENERAL,
}
...
function getLogMeta(level: _LoggerLevel): IBackstageLogMeta {
  return Object.freeze({
    ...
    level:
      ...
      // up front.
      // level &
      // (BackstageLoggerLevel.TRACE |
      //   BackstageLoggerLevel.DEBUG |
      //   BackstageLoggerLevel.WARN |
      //   BackstageLoggerLevel.ERROR),
      hasFlag(level, BackstageLoggerLevel.ERROR)
        ? BackstageLoggerLevel.ERROR
        : hasFlag(level, BackstageLoggerLevel.WARN)
        ? BackstageLoggerLevel.WARN
        : hasFlag(level, BackstageLoggerLevel.DEBUG)
        ? BackstageLoggerLevel.DEBUG
        : // no other option
          BackstageLoggerLevel.TRACE,
  })
 ...
 */
function shouldLog(
  includes: Required<IBackstageLogIncludes>,
  level: _LoggerLevel,
) {
  return (
...
type InternalLoggerRef = {
  ...
  includes: Required<IBackstageLogIncludes>
  filtered: (
    this: IBackstageLogSource,
    level: _LoggerLevel,
    message: string,
    args?: Loggable | (() => Loggable),
  ) => void
  include: (obj: IBackstageLogSource) => IBackstageLogIncludes
  create: (obj: IBackstageLogSource) => ILogger
  creatExt: (obj: IBackstageLogSource) => IBackstageLogger
  style: InternalLoggerStyleRef
  named(
    this: InternalLoggerRef,
    parent: IBackstageLogSource,
    name: string,
    key?: number | string,
  ): ILogger
}
...
/** @internal */
export type IBackstageInternalLogger = {
  configureLogger(config: IBackstageLoggerConfig): void
  configureLogging(config: IBackstageLoggingConfig): void
  getLogger(): ILogger
}

export type IBackstageInternalLoggerOptions = {
  _error?: (message: string, args?: object) => void
  _debug?: (message: string, args?: object) => void
}

export function createBackstageInternalLogger(
  useConsole: IBackstageConsoleLogger = console,
  // Not yet, used, but good pattern to have in case we want to log something
  // or report something interesting.
  _options: IBackstageInternalLoggerOptions = {},
...
function configNamedToKeyed(
  namedFn: (names: string[]) => IBackstageLogger,
  source: IBackstageLogSource,
): IBackstageLogger {
  const names: string[] = []
...
function _createConsoleLogger(
  ...
  source: IBackstageLogSource,
  includes: {min: BackstageLoggerLevel; dev: boolean; internal: boolean},
  con: IBackstageConsoleLogger,
