...
Built from `backstage/studio/src` with `__BACKSTAGE_LITE__` and published from `dist/` (copied from `backstage/studio/dist/index-lite.*` during `yarn workspace backstage build`).
...
The lite bundle replaces full-only modules with stubs (`backstage/devEnv/studioLiteEsbuildStubs.ts`):
...
Runtime behavior still uses `isBackstageLiteStudio()` to block sequencing state edits and hide timeline toolbar controls.
